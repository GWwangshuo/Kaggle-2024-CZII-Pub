...
class VoxResNet(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=7):
        ...
        self.net_c0 = nn.Sequential(
            ...
            nn.PReLU(),
            nn.Conv3d(num_channels, out_channels, 3, 1, 1),
        )
        ...
        self.net_c1 = nn.Sequential(
            ...
            nn.PReLU(),
            nn.Conv3d(num_channels * 2, out_channels, 3, 1, 1),
        )
        self.net_h2 = nn.Sequential(
            nn.InstanceNorm3d(num_channels * 2),
            nn.PReLU(),
    ...
    def _make_layer(self, block, in_planes, planes, num_block):
        ...
        layers.extend(
            [block(planes * block.expansion, planes) for i in range(num_block - 1)]
        )
    ...
    def forward(self, x):
        ...
        c3 = self.net_c3(h)
        c1 = F.interpolate(
            c1, size=c0.shape[-3:], mode="trilinear", align_corners=False
        )
        c2 = F.interpolate(
            c2, size=c0.shape[-3:], mode="trilinear", align_corners=False
        )
        c3 = F.interpolate(
            c3, size=c0.shape[-3:], mode="trilinear", align_corners=False
        )
        c = c0 + c1 + c2 + c3
        c = F.interpolate(
            c, size=original_shape[-3:], mode="trilinear", align_corners=False
        )
